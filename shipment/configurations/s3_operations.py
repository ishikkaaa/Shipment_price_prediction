...
import sys
from io import StringIO #helpful in reading the file contents
from typing import List, Union
from shipment.constants import *
import boto3 #boto3 is Python's official tool to talk to AWS.
...
class S3Operation:
    def __init__(self):
        self.s3_client = boto3.client("s3") #it is used in low level interactions with s3
        self.s3_resource = boto3.resource("s3") #provides high level abstractions for working with buckets and objects
    ...
    def read_object(
        ...
        try:
            func = (
                ...
                if decode is True #means return the object as a string  otherwise it will return in raw form(as a file)
